...
class _MobileRequestRewriteState extends State<MobileRequestRewrite> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: Text(
          localizations.requestRewriteList,
  ...
  void add([int currentIndex = -1]) {
    Navigator.push(
      context,
      MaterialPageRoute(builder: (_) => const RewriteRule()),
    ).then((rule) {
      if (rule != null) {
        setState(() {});
      }
...
class RequestRuleList extends StatefulWidget {
  ...
  RequestRuleList(this.requestRewrites)
    : super(key: GlobalKey<_RequestRuleListState>());
...
class _RequestRuleListState extends State<RequestRuleList> {
  ...
  Future<void> showEdit(int index) async {
    ...
    Navigator.of(context)
        .push(
          MaterialPageRoute(
            builder: (context) => RewriteRule(rule: rule, items: rewriteItems),
          ),
        )
        .then((value) {
        ...
        return Wrap(
          ...
          children: [
            ...
            BottomSheetItem(
              ...
              onPressed: () => export(ctx, [index]),
            ),
            const Divider(thickness: 0.5, height: 5),
            BottomSheetItem(
              text: rules[index].enabled
