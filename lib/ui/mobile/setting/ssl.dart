  ...
  Future<void> _refreshStatus() async {
    ...
    try {
      ...
      if (Platform.isIOS) {
        ...
        logger.d(
          '[HTTPS] iOS CA status: installed=$_installed keychain=$installedByKeychain trusted=$_trusted',
        );
      }
    } catch (e, st) {
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    try {
                      ...
                      logger.e(
                        'import p12 error [$password]',
                        error: e,
                        stackTrace: stackTrace,
                      );
                      if (context.mounted)
                        FlutterToastr.show(localizations.importFailed, context);
                      return;
                    }
                  },
...
class _AndroidCaInstallState extends State<AndroidCaInstall>
    ...
    return ListView(
      ...
      children: [
        ...
        futureWidget(
          ...
          (name) => SelectableText(
            localizations.androidRootRename(name),
        ...
        FilledButton(
          onPressed: () async =>
              _downloadCert(await CertificateManager.systemCertificateName()),
          child: Text(localizations.androidRootCADownload),
        ),
        const SizedBox(height: 10),
        Text(
          isCN
              ? "自动安装（需Root和system写权限，重启生效）"
              : "Auto install (Root & /system write, reboot required)",
  ...
  ListView userCA() {
    ...
    return ListView(
      ...
      children: [
        Text(
          localizations.androidUserCATips,
          style: const TextStyle(fontWeight: FontWeight.w500),
        ),
        const SizedBox(height: 5),
        TextButton(
          ...
          onPressed: () {},
          child: Text(
            "1. ${localizations.downloadRootCa} ",
            textAlign: TextAlign.left,
          ),
        ),
        FilledButton(
          onPressed: () => _downloadCert('ProxyPinCA.crt'),
          child: Text(localizations.downloadRootCa),
        ),
        const SizedBox(height: 5),
        TextButton(
          onPressed: () {},
  ...
  void _downloadCert(String name) async {
    ...
    String? outputFile = await FilePicker.platform.saveFile(
      dialogTitle: 'Please select the path to save:',
      fileName: name,
      bytes: await caFile.readAsBytes(),
    );
  ...
  Future<void> _autoInstallCert() async {
    ...
    try {
      ...
      final shellCmd =
          'cp $caPath $destPath && chmod 644 $destPath && chown root:root $destPath';
...
class IOSCertChecker {
  ...
  static void check(BuildContext context) async {
    ...
    if ((await LocalStorage.getBool(SharedPreferenceKeys.CERT_INSTALL_SKIP)) ==
        true) {
      return;
    }
    final caPem = await CertificateManager.certificatePem();
              ...
              TextButton(
                onPressed: () {
                  LocalStorage.setBool(
                    SharedPreferenceKeys.CERT_INSTALL_SKIP,
                    true,
                  );
                  Navigator.pop(context);
                },
...
class _IosCaInstallState extends State<IosCaInstall> {
  ...
  Future<void> _refreshStatus() async {
    ...
    try {
      certDetails =
          CertificateManager.caCert ??
          await CertificateManager.getCertificateDetails();
      final caPem = await CertificateManager.certificatePem();
      if (Platform.isIOS) {
        trusted = await evaluateChainTrusted(caPem);
  ...
  void _copyProxyLink() async {
    CertificateManager.cleanCache();
    await widget.proxyServer.retryBind();
    var urlStr = Uri.parse(
      "http://127.0.0.1:${widget.proxyServer.port}/ssl",
    ).toString();
  ...
  Widget _statusCard(bool isCN) {
    ...
    if (!installed) {
      ...
      title = isCN ? '证书未安装' : 'Certificate Not Installed';
      subtitle =
          '${localizations.download} & ${localizations.installCaDescribe}';
    } else if (!trusted) {
      color = Colors.orange;
      icon = Icons.warning_amber_rounded;
      title = isCN ? '证书未信任' : 'Certificate Not Trusted';
          ...
          children: [
            ...
            if (!installed) ...[
              ...
              TextButton.icon(
                ...
                label: Text(localizations.downloadRootCaNote),
              ),
            ],
            if (trusted && certDetails != null) ...[
              const Divider(height: 12),
              _certDetails(certDetails!),
            ],
          ],
        ),
      ),
  ...
  Widget _certDetails(X509CertificateData details) {
    final infoLabelStyle = Theme.of(
      context,
    ).textTheme.bodySmall?.copyWith(color: Colors.grey[600]);
    final infoValueStyle = Theme.of(
      context,
    ...
    return Column(
      children: [
        ...
        _kv(
          'Name',
          details.subject['2.5.4.3'] ?? 'ProxyPin CA',
          infoLabelStyle,
          infoValueStyle,
        ),
        const SizedBox(height: 6),
        _kv(
          'Expires',
          details.validity.notAfter.toLocal().toString().split(' ').first,
          infoLabelStyle,
          infoValueStyle,
        ),
        // const SizedBox(height: 6),
    ...
    return Column(
      ...
      children: [
        ...
        TextButton(
          onPressed: () {},
          child: Text(
            "2. ${localizations.installRootCa} -> ${localizations.trustCa}",
          ),
        ),
        TextButton(
          onPressed: () {},
        ...
        Padding(
          ...
          child: Image.network(
            "https://foruda.gitee.com/images/1689346516243774963/c56bc546_1073801.png",
            height: 400,
          ),
        ),
        TextButton(
        ...
        Padding(
          padding: const EdgeInsets.only(left: 15),
          child: Image.network(
            "https://foruda.gitee.com/images/1689346614916658100/fd9b9e41_1073801.png",
            height: 270,
          ),
        ),
      ],
