...
class PCCertChecker {
  ...
  static void check(BuildContext context) async {
    ...
    if (ProxyServer.current?.enableSsl != true ||
        ProxyServer.current?.configuration.enableSystemProxy != true) {
      return;
    }
...
class _PCCertState extends State<PCCert> with TickerProviderStateMixin {
    ...
    return SimpleDialog(
      ...
      children: [
        ...
        SizedBox(
          ...
          child: TabBarView(
            controller: _tabController,
            children: [_AutomaticInstall(), _buildManualTab(context)],
          ),
        ),
      ],
    );
  ...
  List<Widget> _buildWindowsAndMacContent(BuildContext context) {
    ...
    return [
      isCN
          ? Text(
              " 安装证书到本系统，${Platform.isMacOS ? "安装完双击选择“始终信任此证书”。 如安装打开失败，请导出证书拖拽到系统证书里" : "选择“受信任的根证书颁发机构”"}",
            )
          : Text(
      ...
      const SelectableText.rich(
        ...
        TextSpan(
          ...
          children: [
            TextSpan(
              text:
                  "  sudo cp ProxyPinCA.crt /usr/local/share/ca-certificates/ \n",
            ),
...
class _AutomaticInstallState extends State<_AutomaticInstall> {
  ...
  void initState() {
    ...
    if (certDetails == null) {
      CertificateManager.getCertificateDetails().then(
        (value) => setState(() {
          certDetails = value;
        }),
      );
  ...
  void _checkCertStatus() async {
    final details =
        certDetails ?? await CertificateManager.getCertificateDetails();
    final caFile = await CertificateManager.certificateFile();
    isCertInstalled.value = await CertInstaller.isCertInstalled(
      caFile,
      details,
    );
  }
    ...
    return Padding(
      ...
      child: Obx(
        () => Column(
          ...
          children: buildAutomaticChildren(),
        ),
      ),
    );
  }
  ...
  List<Widget> buildAutomaticChildren() {
    ...
    if (isCertInstalled.value == false) {
      ...
      children.add(
        Text(
          localizations.certNotInstalled,
          textAlign: TextAlign.center,
          ...
          child: Padding(
            ...
            child: Column(
              children: [
                ...
                Text(
                  isCN ? "证书已安装" : "Certificate Installed",
                  style: TextStyle(fontSize: 18, fontWeight: FontWeight.w600),
                ),
                const SizedBox(height: 8),
                if (certDetails != null) ...[
                    ...
                    children: [
                      ...
                      Expanded(
                        child: SelectableText(
                          certDetails!.subject['2.5.4.3'] ?? 'ProxyPin CA',
  ...
  void _installCert(BuildContext context) async {
    ...
    if (success) {
      ...
      CustomToast.success(
        isCN ? "证书安装成功" : "Certificate installed successfully",
      ).show(context);
    } else {
