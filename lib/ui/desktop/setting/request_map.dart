...
class _RequestMapPageState extends State<RequestMapPage> {
  ...
  bool onKeyEvent(KeyEvent event) {
    ...
    if ((HardwareKeyboard.instance.isMetaPressed ||
      ...
      HardwareKeyboard.instance.removeHandler(onKeyEvent);
      if (_refresh) {
        _refreshConfig(force: true).whenComplete(
  ...
  Future<void> showEdit() async {
    showDialog(
      ...
      context: context,
      builder: (_) => RequestMapEdit(windowId: widget.windowId),
    ).then((value) {
      if (value != null) {
        setState(() {});
      }
  ...
  void showMenus(TapDownDetails details, int index) {
    ...
    showContextMenu(
      ...
      items: [
        ...
        PopupMenuItem(
          height: 35,
          child: widget.list[index].enabled
...
class _RequestMapEditState extends State<RequestMapEdit> {
  ...
  void initState() {
    ...
    rule =
        widget.rule ??
        RequestMapRule(url: widget.url ?? '', type: RequestMapType.local);
    mapType = rule.type;
