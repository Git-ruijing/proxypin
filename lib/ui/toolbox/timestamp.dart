...
class _TimestampPageState extends State<TimestampPage> {
  ...
  void initState() {
    ...
    nowTimestamp.text = (DateTime.now().millisecondsSinceEpoch ~/ 1000)
        .toString();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text(localizations.timestamp, style: TextStyle(fontSize: 16)),
        centerTitle: true,
      ),
      body: ListView(
                ...
                Expanded(
                  child: Column(
                    children: [
                      ...
                      Row(
                        mainAxisAlignment: MainAxisAlignment.spaceBetween,
                        children: [timeButton(), timeOutCopyButton()],
                      ),
                      SizedBox(height: 5),
                      timeOutField(),
                    ],
  ...
  Widget timestampLabel() {
    return SizedBox(
      width: isCN ? 60 : 93,
      child: Text('${localizations.timestamp}:', style: textStyle),
    );
  }
  ...
  Widget timeLabel() {
    return SizedBox(
      width: isCN ? 60 : 93,
      child: Text('${localizations.time}:', style: textStyle),
    );
  }
  ...
  timestampConvert(String timestamp) {
    ...
    try {
      if (timestamp.length == 13) {
        timestampOut.text = DateTime.fromMillisecondsSinceEpoch(
          int.parse(timestamp),
        ).format();
        return;
      }
